			...
			$.ajax({
				...
				success: function(data){
					...
					for(var i in sc_arr){
						...
						sum += pic1 * sc_arr[i].num;
						html += "<div><img src=" + res[sc_arr[i].id - 1].img + " /><p><span>" + res[sc_arr[i].id - 1].name + "</span><span>满150包邮</span><span>颜色：漆色</span></p><p>" + pic1 + "</p><p>" + sc_arr[i].num + "</p><p>" + pic1 * sc_arr[i].num + "</p><p class =remove value =" + sc_arr[i].id + ">删除</p></div>";
					}
					$(".goodsCart").html(html);
